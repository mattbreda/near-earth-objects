...
const Asteroid = (props) => {
  ...
  const infos = props.legendLabel ? null : (
    <div className={classes['Asteroid--info']}
      style={infoStyle}
      >
      <span className={classes['Asteroid--single-info']}> Name: {uniformName(props.asteroidData.name)} </span>
      <span className={classes['Asteroid--single-info']}> Diameter: {Math.round(props.asteroidDiameter * 100) / 100} km</span>
      <span className={classes['Asteroid--single-info']}> Magnitude: {props.asteroidData.absolute_magnitude_h} h</span>
      <span className={classes['Asteroid--single-info']}> Distance: {Math.round(props.asteroidDistance * 100) / 100} au</span>
      <span className={classes['Asteroid--single-info']}> Velocity: {Math.round(props.asteroidSpeed * 100) / 100} km/s</span>
    </div>
  )
  return (
    <div 
